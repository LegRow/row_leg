require 'rails_helper'

RSpec.describe "newebpay" do

  include Capybara::DSL

  subject(:call) { post endpoint, params: params }
  let(:endpoint) { '/cashflow/from_newebpay' }
  let(:params) do # 這之後應該要由藍新打過來，ngrok
    {
    "Status"=>"SUCCESS",
    "MerchantID"=>"MS119996394",
    "Version"=>"1.5",
    "TradeInfo"=>"",
    "TradeSha"=>"365BDFD74B7B23BA012E77165A57361634742B15DBDD3F4666DC01290C4C0991"
    }
  end

  # context "when sending to newebpay" do
  #   it "successfully encodes" do

  #   end
  # end

  # # posts from newebpay
  # context "when newebpay posts" do
  #   it "returns success response" do
  #     call
  #     expect(response).to be_successful
  #   end
  # end

  # # get to newebpay
  # context "when we pay" do
  #   it ""
  # end

  # # all other methods to use posts from newebpay and get to newebpay
  # context "when employer create task" do
  #   it "successfully pay" do
  #     # arrange: go to "./tasks/new" -> input fake data -> click "新增"
  #     # actions: redirect to "newebpay"
  #     # assertion: get the post of success
  #   end
  # end

  # context "when employer click pay" do
  #   it "successfully pay" do
  #     # arrange: go to "./tasks" -> click pay
  #     # actions: redirect to "newebpay"
  #     # assertion: get the post of success
  #   end
  # end

  # context "when employee click apply" do
  #   it "successfully pay" do
  #   end
  # end

end
