...
document.addEventListener('turbolinks:load', () => {
  ...
  const employeeMapOption = { // 地圖初始化設定：位置與縮放
    zoom: 14,
    center: {lat: 25.009571560497424, lng: 121.46213302250602}, // 這邊之後也要填入店家位置
  };
  
  const locationChannel = consumer.subscriptions.create({ channel: "LocationsChannel", employeeId: employeeId }, {
  ...
  if (currentUser === employeeId) {
    ...
    reportLocation.addEventListener('click', (e) => {
      ...
      // call google map
      const currentPosition = navigator.geolocation.getCurrentPosition(succeed, fail)
      ...
      function succeed(position) {
        ...
        locationChannel.speak({employeeLocation: employeeLocation, sentBy: employeeId, shouldBeReceivedBy: employerId });
